            ...
            let mut meshes: Vec<rustlight::geometry::Mesh> = scene_info
                ...
                .map(|m| match m.data {
                    pbrt_rs::Shape::TriMesh(ref data) => {
                        ...
                        let bsdf = if let Some(ref name) = m.material_name {
                            if let Some(bsdf_name) = scene_info.materials.get(name) {
                                ...
                                Box::new(rustlight::bsdfs::diffuse::BSDFDiffuse {
                                    diffuse: rustlight::bsdfs::BSDFColor::UniformColor(Color::value(0.8)),
                                })
                            }
                        } else {
                            Box::new(rustlight::bsdfs::diffuse::BSDFDiffuse {
                        ...
                        let mut mesh = Mesh::new("noname".to_string(), points, indices, normals, uv);
                        mesh.bsdf = bsdf;
                        mesh
                    }
                })
    ...
    pub fn get_img(&self, ctx: &CanvasRenderingContext2d) -> Result<(), JsValue> {
        ...
        for i in 0..(self.width*self.height) as usize {
            if self.nb_samples[i] != 0 {
                ...
                data[i*4+0] = ((self.img[i].r * inv_nb_samples).powf(1.0/2.2) * 255.0) as u8;
                data[i*4+1] = ((self.img[i].g * inv_nb_samples).powf(1.0/2.2) * 255.0) as u8;
                data[i*4+2] = ((self.img[i].b * inv_nb_samples).powf(1.0/2.2) * 255.0) as u8;
                data[i*4+3] = 255;
            } else {
                data[i*4+0] = 0;
                data[i*4+1] = 0;
